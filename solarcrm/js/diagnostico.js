// ============================================================
//  SolarCRM — Diagnóstico inteligente de alertas v2
//  Separa OFFLINE (sem comunicação) de ALARME (falha interna)
// ============================================================
...
const Diagnostico = (() => {
  ...
  function analisar(cliente) {
    ...
    // Tipo real: alarme vs offline
    const tipoReal = cliente.tipoAlerta || 'offline'; // 'alarme' | 'offline'

    let causa, descricao, severidade, acoes;

    if (tipoReal === 'alarme') {
      // ── ALARME: inversor comunicando mas com falha interna ──
      causa      = 'Alarme ativo no inversor';
      descricao  = `O inversor está se comunicando normalmente, mas reportou uma falha interna. Isso pode indicar sobretensão, subtensão da rede, temperatura elevada, falha nos painéis ou erro de hardware. Verifique o display do inversor para o código de erro específico.`;
      severidade = 'err';
      acoes      = [
      ...
      if (eDay === 0 && eMes === 0 && eTotal > 0 && diasSemComm && diasSemComm > 30) {
        causa      = 'Sem geração e comunicação há mais de 30 dias';
        descricao  = 'O sistema não registra geração ou comunicação há mais de um mês. Possível desligamento da instalação, falha grave no inversor ou problema na rede elétrica.';
        severidade = 'err';
        acoes      = ['Visita técnica urgente', 'Verificar se instalação ainda está ativa', 'Checar disjuntores e fusíveis'];

      } else if (eDay === 0 && diasSemComm && diasSemComm > 7) {
        ...
        causa      = 'Inversor offline';
        descricao  = 'O inversor perdeu comunicação com a plataforma de monitoramento. Verifique a conexão de rede do datalogger.';
        severidade = 'warn';
        acoes      = ['Verificar conexão de rede do inversor', 'Reiniciar datalogger', 'Contatar cliente'];
      }
    }
    ...
    return { causa, descricao, severidade, acoes, tempoStr, horasSemComm, diasSemComm, eDay, eMes, tipoReal };
  }
  ...
  function renderModal(alerta, cliente) {
    ...
    // Cor e ícone baseados no TIPO REAL do alerta
      ...
      <div class="modal" style="max-width:560px;">
        ...
        <div style="padding:16px 20px;display:flex;flex-direction:column;gap:14px;">
          ...
          <div style="background:${sevColor}18;border:1px solid ${sevColor}44;border-radius:10px;padding:14px;">
            <div style="font-size:13px;font-weight:700;color:${sevColor};margin-bottom:6px;">${diag.causa}</div>
